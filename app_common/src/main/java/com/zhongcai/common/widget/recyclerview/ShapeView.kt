...
class ShapeView(ctx: Context,attrs: AttributeSet?): View(ctx,attrs){
    ...
    init {
        ...
        radius = typedArray.getDimension(R.styleable.shapeview_sv_radius, DensityUtils.dp2px(radius).toFloat())
        soild = typedArray.getColor(R.styleable.shapeview_sv_soild_color,
                BaseUtils.getColor(R.color.transparent))
        strokeColor = typedArray.getColor(R.styleable.shapeview_sv_stoke_color,
                BaseUtils.getColor(R.color.transparent))
    ...
    private fun drawStrokeLine(canvas: Canvas) {
        if (strokeWidth > 0) {
            ...
            paint?.let{
                canvas.drawPath(strokeWidthPath, it)
            }
        }
    }
